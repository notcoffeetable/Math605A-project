...
\huge Key Definitions \\ % The assignment title
...
\item $v(a) =0$ for any nonzero unit.
\end{enumerate}
\end{defn}
\begin{prop}
...
Consider now $K$, the fraction field of a Dedekind ring $O_K$, with valuation $\nu$ and uniformizing parameter $\pi_K$. Also consider the Galois extension $L/K$ with Galois group $G$; prolongation of $\nu$, $\omega$, on $L$; uniformizing parameter $\pi_L$ of $\omega$; and discrete valuation ring $O_L$. For every integer $i \geq -1$ define the $i^{th}$ ramification group
$$
G_i = \{\sigma \in G | \omega(\sigma(a)-a)\geq i+1 \;\forall a \in O_L\}.
$$
\end{defn}
